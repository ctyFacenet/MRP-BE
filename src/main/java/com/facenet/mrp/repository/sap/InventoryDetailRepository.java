...
public interface InventoryDetailRepository extends JpaRepository<OwhsEntity,Long> {
    @Query(value = "select new com.facenet.mrp.service.dto.InventoryDetailDTO(a.whsCode,a.whsName,b.onHand,b.onOrder,b.minStock,b.maxStock) from OwhsEntity as a join OitwEntity as b on a.whsCode = b.whsCode where b.itemCode = :ItemCode and b.whsCode in :listWarehouse")
    List<InventoryDetailDTO> getInventoryDetailByItemCode(@Param("ItemCode") String ItemCode, @Param("listWarehouse") List<String> listWarehouse);
