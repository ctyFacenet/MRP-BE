...
//            result.setSumPrQuantity(planRepository.sumPrQuantity(result.getItemCode(), result.getPurchaseRecommendationId()));
    ...
    public PageResponse<List<ReportXPDTO>> getSOReport(PageFilterInput<ReportXPDTO> input, Pageable pageable)
    {
        ...
        sql.append("SELECT ")
            ...
            .append("LEFT JOIN purchase_request pr ON pro.product_order_code = pr.so_code ")
            .append("LEFT JOIN purchase_request_detail pr_detail ON pr.pr_code = pr_detail.pr_code ")
            .append("LEFT JOIN purchase_order_purchase_request pur_pr ON FIND_IN_SET(pr.pr_code, pur_pr.purchase_request_code) ")
